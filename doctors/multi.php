...
//        $iblockId = Iblock::$dataClass::getList(['filter' => ['API_CODE' => 'Doctors']])->fetch()['ID'];
//        /** @var $dataClass \Bitrix\Main\ORM\Data\DataManager*/
//        $dataClass = Iblock::wakeUp($iblockId)->getEntityDataClass();
//       var_dump($dataClass);

//$docId = 74; // идентификатор доктора из инфоблока Доктора
$doctors = ElementDoctorsTable::getList([ // получение списка процедур у врачей
    'select' => [
        'ID', 
        'NAME', 
        'SPETSIALIZATSIYA.ELEMENT.NAME',
        'FIO',
//        'PROPERTY_SPETSIALIZATSIYA_DESCRIPTION' // PROC_IDS_MULTI - множественное поле Процедуры у элемента инфоблока Доктора
    ], 
    'filter' => [
      //  'ID' => $docId,
        'ACTIVE' => 'Y',
...
foreach ($doctors as $doctor) {
    ...
    pr($doctor->getId().' '. $doctor->getName().' '. $doctor->getFio()->getValue());

    foreach($doctor->getSpetsializatsiya()->getAll() as $prItem) {
